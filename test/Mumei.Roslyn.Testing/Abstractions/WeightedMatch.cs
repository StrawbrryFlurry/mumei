...
internal static class WeightedMatch {
    ...
    /// <param name="matchers">
    ///   A collection of matching functions. Each matching function returns 0 to indicate a
    ///   match, or a value greater than 0 for mismatch. Greater values are further from matching.
    /// </param>
    /// <param name="matchTimeout">
    ///   The maximum time to spend searching for the best match in the event no exact
    ///   match exists.
    /// </param>
    /// <returns>
    ...
    ///     <item>
    ///       <description>
    ///         Every element of <paramref name="expected" /> will appear exactly once as the expected element of an
    ///         item in the result.
    ///       </description>
    ///     </item>
    ///     <item>
    ///       <description>
    ///         Every element of <paramref name="actual" /> will appear exactly once as the actual element of an
    ///         item in the result.
    ///       </description>
    ///     </item>
    ///     <item>
    ///       <description>
    ///         An item in the result which specifies both an expected and an actual value indicates a matched pair,
    ///         i.e. the actual and expected results are believed to refer to the same item.
    ///       </description>
    ///     </item>
    ///     <item>
    ///       <description>
    ///         An item in the result which specifies only an actual value indicates an item for which no matching
    ///         expected item was found.
    ///       </description>
    ///     </item>
    ///     <item>
    ///       <description>
    ///         An item in the result which specifies only an expected value indicates an item for which no matching
    ///         actual item was found.
    ///       </description>
    ///     </item>
    ///   </list>
    ///   <para>
    ///     If no exact match is found (all actual diagnostics are matched to an expected diagnostic without
    ...
    var resultBuilder =
      ImmutableArray.CreateBuilder<Result<TExpected, TActual>>(Math.Max(expected.Length, actual.Length));
    MatchRecursive(
      expected,
      actual,
      matchers,
    ...
    for (var i = 0; i < commonCount; i++) {
      var distance = Evaluate(expected[i], actual[i], false, matchers);
      distanceCache[i * actual.Length + i] = distance;
      resultBuilder.Add(new Result<TExpected, TActual>(true, expected[i], true, actual[i], distance));
    }

    for (var i = commonCount; i < expected.Length; i++) {
      resultBuilder.Add(new Result<TExpected, TActual>(true, expected[i], false, default, 0));
    }

    for (var i = commonCount; i < actual.Length; i++) {
      resultBuilder.Add(new Result<TExpected, TActual>(false, default, true, actual[i], 0));
    }
  ...
  private static void MatchRecursive<TExpected, TActual>(
    ...
    for (var i = 0; i < usedActual.Length; i++) {
      ...
      try {
        usedActual[i] = true;
        resultBuilder.Add(new Result<TExpected, TActual>(true, expected[nextExpected], true, actual[i], distance));
        MatchRecursive(
          expected,
          actual,
          matchers,
    ...
    if (remainingUnmatched > 0 && expected.Length > actual.Length) {
      ...
      try {
        resultBuilder.Add(new Result<TExpected, TActual>(true, expected[nextExpected], false, default, 0));
        MatchRecursive(
          expected,
          actual,
          matchers,
